...
/**
 * The CRC16 hash is a relatively quick algorithm and provides for a
 * "more random" distribution or elements across nodes.
 */
public class CRC16CacheletHash extends ICacheletHash {
	...
	public void getCacheletsFromKey(String key, Set<Integer> set,
		...
		for (i = 1; i <= replication; ++i) {
			input = input + crc16(input);
			crc = crc16(input) % numCachelets;
			set.add(crc);
			if (set.size() != i) {
				--i;
			}
