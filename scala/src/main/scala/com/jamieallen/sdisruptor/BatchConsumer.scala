 ...
 *
 *  param  Entry implementation storing the data for sharing during exchange or parallel coordination of an event.
 */
class BatchConsumer[A <: AbstractEntry](consumerBarrier: ConsumerBarrier[A], handler: BatchHandler[A]) extends Consumer {
   ...
   *
   *  @param newExceptionHandler to replace the existing exceptionHandler.
   */
