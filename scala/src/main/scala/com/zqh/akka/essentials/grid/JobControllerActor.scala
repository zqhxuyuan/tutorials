  ...
  def receive: Receive = {
    ...
    case SendWork =>
      ...
      if (workerRouterActor != null) {
        ...
        // tell the workscheduler actor to wake me up in 5 millisecs again
        workSchedulerActor.tell(5000, self)
      }
    case ENOUGH =>
      log.info("Worker requesting shutdown" + sender.path.toString())
      // Shut down the routee at this point and remote the routerconfig
